    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-6">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-zinc-500">Okręg:</span>
              <p className="text-zinc-900">{mp.districtName} ({mp.districtNum})</p>
            </div>
            <div>
              <span className="text-zinc-500">Województwo:</span>
              <p className="text-zinc-900">{mp.voivodeship}</p>
            </div>
            {mp.profession && (
              <div>
                <span className="text-zinc-500">Zawód:</span>
                <p className="text-zinc-900">{mp.profession}</p>
              </div>
            )}
            {mp.educationLevel && (
              <div>
                <span className="text-zinc-500">Wykształcenie:</span>
                <p className="text-zinc-900">{mp.educationLevel}</p>
              </div>
            )}
            {mp.birthDate && (
              <div>
                <span className="text-zinc-500">Data urodzenia:</span>
                <p className="text-zinc-900">{formatDate(mp.birthDate)}</p>
              </div>
            )}
            {mp.numberOfVotes && (
              <div>
                <span className="text-zinc-500">Liczba głosów:</span>
                <p className="text-zinc-900 font-medium">{mp.numberOfVotes.toLocaleString('pl-PL')}</p>
              </div>
            )}
            {mp.email && (
      ...
      {mp.votingStats && (
        <div className="rounded-lg border border-zinc-200 p-4">
          <h2 className="text-sm font-medium text-zinc-900 mb-4">Statystyki głosowań</h2>

          {/* Attendance bar */}
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-1">
              <span className="text-zinc-600">Frekwencja</span>
